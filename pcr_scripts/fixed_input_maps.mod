...
# 1 make homogeneous maps: thetas1, thetas2, psi1, psi2, ksat2
# 2 make landuse specific maps: lai, smax, ch, cover, rr, thetai2
...
report soildep1 = lookupscalar(homtbl, 4, area);
report soildep2 = lookupscalar(homtbl, 5, area);
report psi2 = lookupscalar(homtbl, 7, area);
...
report	rr_temp = lookupscalar(croptbl, 5, landuse);
	aggr = lookupscalar(croptbl, 7, landuse);
report thetai2 = lookupscalar(croptbl, 6, landuse);
...
report rr_comp = (comp_frac * 0.5 * rr_temp) + ((1 - comp_frac) * rr_temp);
...
report	rr = rr_comp + thalwegrr;
	...
	channel_slope = lookupscalar(homtbl, 6, area);
