...
# the resulting maps are stored online in the folder './spatial_data'
...
# 3. base_maps_subcatch.mod    = cut other base maps to correct size.
...
base_maps_subcatchment(cell_size = 20, sub_catch_number = 18, run_type = "cal", calc_ldd = T)
...
#'      multiplication factors for each landuse can be changed in
#'      ./sources/setup/calibration/calibration_landuse.csv
#' 2. the resulting swatre input parameters from section 2.2 can be adjusted 
#'      for each UBC code. Adjustments can be made in:
#'      ./sources/setup/calibration/calibration_soil.csv
#' 3. the channel mannings n properties: see sources/setup/tables/
#' 4. initial soil moisture conditions can also be calibrated:
#'      ./sources/setup/calibration/calibration_theta.csv
#' 
...
#' 4. baseflow_calibration.mod  = calculates baseflow for subcatchments for each event
...
# you can also run for one specific subcatchment e.g.
create_lisem_run(resolution = 20, catch_num = 18, swatre_file = swatre_file, run_type = "cal", T)
...
## 3.2 Update database including NBS ------------------------------------------

# from here on we will start including NBS measure to the lisem simulations
...
## 3.2 Make a lisem run with a specific NBS measure or scenario ----------------
